...
function titleCase(value) {
  return value.replace(/^_+|_+$/g, "").replace(/([a-z])([A-Z])/g, "$1 $2").replace(/[._-]+/g, " ").replace(/\b(app|ai|saas|seo|crm|it|ryk|pk|uk|usa|llc)\b/gi, (match) => match.toUpperCase()).replace(/\b\w/g, (letter) => letter.toUpperCase());
}
...
function buildEntry(raw, category) {
  ...
  const linkType = platform === "Facebook" || platform === "Instagram" || platform === "LinkedIn" ? "social" : isStaging ? "staging" : requiresLogin ? "login" : "live";
  return {
...
].filter((entry) => !entry.isStaging && !entry.requiresLogin).filter((entry, index, entries) => entries.findIndex((item) => portfolioKey(item.url) === portfolioKey(entry.url)) === index);
var portfolioStats = {
