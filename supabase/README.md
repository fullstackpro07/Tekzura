# calderforge Lead Capture Backend

The Get Started form (`/get-started`) uses a lean, no-server lead architecture:

- **Web3Forms** sends email notifications to calderforge.
...
email to `info@calderforge.com`, so no lead is lost.
...
Create or verify the Web3Forms access key using `info@calderforge.com`. Web3Forms
delivers notifications to the email address connected to the access key, so the
key itself must be tied to calderforge's notification inbox.
...
`info@calderforge.com` and insert a row in Supabase. If Web3Forms is configured but
